"""arcgis_mcp.setup_env — Stage 1 administrative bootstrap for the ArcGIS MCP Bridge.
...
------------------------------
This module performs NO current-working-directory-relative file creation:
the conda executable is resolved via ``ARCGIS_CONDA_EXE`` / PATH / known
ArcGIS Pro install locations, the environment clone lands wherever conda
manages environments, logging goes to stderr, and the single stdout product
is the JSON report. It is therefore safe to invoke from any directory —
including globally as the ``arcgis-mcp-setup`` console script.
