...
def _create_feature_class(arcpy: Any, inp: c.CreateFeatureClassInput) -> dict:
    ...
    res = arcpy.management.CreateFeatureclass(   # exact Esri casing
        out_path=inp.out_gdb, out_name=inp.name,
        geometry_type=inp.geometry_type, spatial_reference=sr,
    )
    return {"created": str(res), "geometry_type": inp.geometry_type,
            "wkid": inp.wkid}
...
def _add_field(arcpy: Any, inp: c.AddFieldInput) -> dict:
    arcpy.management.AddField(
        in_table=inp.dataset, field_name=inp.field_name,
        field_type=inp.field_type, field_length=inp.length,
...
def _calculate_field(arcpy: Any, inp: c.CalculateFieldInput) -> dict:
    arcpy.management.CalculateField(
        in_table=inp.dataset, field=inp.field_name,
        expression=inp.expression, expression_type=inp.expression_type,
    )
...
def _add_fields_batch(arcpy: Any, inp: c.AddFieldsBatchInput) -> dict:
    descriptions = [
        [f.name, f.type, f.alias or f.name, f.length, None, None]
        for f in inp.fields
    ]
...
def _get_field_info(arcpy: Any, inp: c.GetFieldInfoInput) -> dict:
    return {"fields": [
        {"name": f.name, "type": f.type, "length": f.length,
         "alias": f.aliasName, "nullable": f.isNullable}
...
def _describe_dataset(arcpy: Any, inp: c.DescribeDatasetInput) -> dict:
    ...
    return {
        "name": d.name, "data_type": d.dataType,
        "shape_type": getattr(d, "shapeType", None),
        "crs": (f"EPSG:{sr.factoryCode}" if sr and sr.factoryCode
                else (sr.name if sr else None)),
        "extent": ({"xmin": ext.XMin, "ymin": ext.YMin,
                    "xmax": ext.XMax, "ymax": ext.YMax} if ext else None),
    }
...
def _export_to_geojson(arcpy: Any, inp: c.ExportToGeojsonInput) -> dict:
    ...
    arcpy.conversion.FeaturesToJSON(
        in_features=inp.in_features, out_json_file=inp.out_json,
...
def _get_extent(arcpy: Any, inp: c.GetExtentInput) -> dict:
    ...
    return {"xmin": ext.XMin, "ymin": ext.YMin,
