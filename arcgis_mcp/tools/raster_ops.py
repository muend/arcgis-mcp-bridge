...
def _extract_by_mask(arcpy: Any, inp: c.ExtractByMaskInput) -> dict:
    with _extension(arcpy, "Spatial"):
        return _save(arcpy.sa.ExtractByMask(inp.in_raster, inp.mask),
                     inp.out_raster)
...
def _zonal_statistics(arcpy: Any, inp: c.ZonalStatisticsInput) -> dict:
    with _extension(arcpy, "Spatial"):
        result = arcpy.sa.ZonalStatistics(
            inp.in_zone_data, inp.zone_field, inp.in_value_raster,
            inp.statistics_type)
        out = _save(result, inp.out_raster)
...
def _zonal_statistics_as_table(
    arcpy: Any, inp: c.ZonalStatisticsAsTableInput
) -> dict:
    with _extension(arcpy, "Spatial"):
        arcpy.sa.ZonalStatisticsAsTable(
            inp.in_zone_data, inp.zone_field, inp.in_value_raster,
            inp.out_table, "DATA", inp.statistics_type)
    return {"output": inp.out_table, "statistic": inp.statistics_type}


def _slope(arcpy: Any, inp: c.SlopeAnalysisInput) -> dict:
    with _extension(arcpy, "Spatial"):
        result = arcpy.sa.Slope(inp.in_raster, inp.output_measurement,
                                inp.z_factor)
        out = _save(result, inp.out_raster)
...
def _hillshade(arcpy: Any, inp: c.HillshadeInput) -> dict:
    with _extension(arcpy, "Spatial"):
        result = arcpy.sa.Hillshade(
            inp.in_raster, inp.azimuth, inp.altitude,
            "SHADOWS" if inp.model_shadows else "NO_SHADOWS", inp.z_factor)
        out = _save(result, inp.out_raster)
...
def _contour_lines(arcpy: Any, inp: c.ContourLinesInput) -> dict:
    with _extension(arcpy, "Spatial"):
        arcpy.sa.Contour(inp.in_dem, inp.out_features, inp.contour_interval,
                         inp.base_contour, inp.z_factor)
...
def _resample_raster(arcpy: Any, inp: c.ResampleRasterInput) -> dict:
    arcpy.management.Resample(inp.in_raster, inp.out_raster,
                              str(inp.cell_size), inp.resampling_type)
    return {"output": inp.out_raster, "cell_size": inp.cell_size,
...
def _mosaic_to_new_raster(arcpy: Any, inp: c.MosaicToNewRasterInput) -> dict:
    ...
    return {"output": f"{inp.output_location}/{inp.raster_name}",
...
def _raster_to_polygon(arcpy: Any, inp: c.RasterToPolygonInput) -> dict:
    arcpy.conversion.RasterToPolygon(
        inp.in_raster, inp.out_features,
        "SIMPLIFY" if inp.simplify else "NO_SIMPLIFY", inp.raster_field)
    return {"output": inp.out_features}


def _polygon_to_raster(arcpy: Any, inp: c.PolygonToRasterInput) -> dict:
    arcpy.conversion.PolygonToRaster(
        inp.in_features, inp.value_field, inp.out_raster,
        inp.cell_assignment, cellsize=inp.cell_size)
    return {"output": inp.out_raster, "value_field": inp.value_field}


def _clip_raster(arcpy: Any, inp: c.ClipRasterInput) -> dict:
    rectangle = (
        f"{inp.xmin} {inp.ymin} {inp.xmax} {inp.ymax}"
        if inp.xmin is not None else "#"
    )
    arcpy.management.Clip(
        ...
        clipping_geometry=(
            "ClippingGeometry" if inp.use_clipping_geometry else "NONE"),
    )
    return {"output": inp.out_raster, "rectangle": rectangle,
